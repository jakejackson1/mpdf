...
class BaseTagTestCase extends TestCase
{
	...
	protected function getService($serviceName)
	{
		...
		// If not found via Reflection (e.g. dynamic property not yet set or invisible to reflection if not declared?),
		// try accessing it directly if it's dynamic and public (though Strict trait prevents this for external access usually)
		// But Strict trait prevents __get.
		// If it's a dynamic property, ReflectionObject (not ReflectionClass) should find it if it's set.
		
		$reflectionObject = new \ReflectionObject($this->mpdf);
		if ($reflectionObject->hasProperty($serviceName)) {
