...
/**
 * Comprehensive unit tests for CssManager class
 * Tests all 23 public and protected methods
 */
class CssManagerTest extends \Yoast\PHPUnitPolyfills\TestCases\TestCase
{
    ...
    public function testReadCSS_WithBackgroundUrlRewriting()
    {
        ...
        // URL rewriting might happen inside ReadCSS logic.
        // Mpdf usually rewrites relative URLs in CSS to absolute or relative to basepath.
        // Since we set basepath to tempDir, it should be resolved.
    ...
    public function testReadCSS_WithDataUri()
    {
        ...
        // Real Mpdf converts data URIs to temp files for processing
        $this->assertStringContainsString('link', $result);

        $this->assertArrayHasKey('CLASS>>ICON', $this->cssManager->CSS);
        // The real implementation converts data URIs to temp file paths
        $this->assertArrayHasKey('BACKGROUND-IMAGE', $this->cssManager->CSS['CLASS>>ICON']);
        // Just verify an image path is set (could be temp file or data URI depending on implementation)
    ...
    public function testReadCSS_ComprehensiveStressTest()
    {
        ...
        $this->createCssFile('main.css', '@import url("base.css");
        ...
        $this->assertStringNotContainsString('<!-- External Stylesheets', $result);
        $this->assertStringNotContainsString('<!-- Inline Styles', $result);
        $this->assertStringNotContainsString('<!-- Print Styles', $result);

        // Result should contain the processed HTML structure
        ...
        // Verify link tags for external stylesheets are PRESERVED
        $this->assertStringContainsString('<link', $result);
        ...
        // Verify some CSS rules were parsed (exact keys may vary based on parsing)
        // Just check that BODY or some block-level element is present
        $hasRules = isset($this->cssManager->CSS['BODY']) ||
                    isset($this->cssManager->CSS['HTML']) ||
                    isset($this->cssManager->CSS['CLASS>>BTN-PRIMARY']) ||
                    count($this->cssManager->CSS) > 0;
        $this->assertTrue($hasRules, 'Expected CSS to be parsed with some rules');
    }
    ...
    // ========== array_merge_recursive_unique() Tests ==========
    ...
    protected function set_up()
    {
        ...
        // Ensure we have a page to work with, as some operations might check for it
        $this->mpdf->AddPage();
    }
