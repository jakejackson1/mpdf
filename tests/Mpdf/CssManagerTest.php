...
class CssManagerTest extends \Yoast\PHPUnitPolyfills\TestCases\TestCase
{
	...
	{
		$result   = $this->invokeMethod($this->cssManager, 'expandShorthandProperty', ['10px']);
	...
	{
		$result   = $this->invokeMethod($this->cssManager, 'expandShorthandProperty', ['10px 20px']);
	...
	{
		$result   = $this->invokeMethod($this->cssManager, 'expandShorthandProperty', ['10px 20px 30px']);
	...
	{
		$result   = $this->invokeMethod($this->cssManager, 'expandShorthandProperty', ['10px 20px 30px 40px']);
	...
	{
		$result   = $this->invokeMethod($this->cssManager, 'expandShorthandProperty', ['10px 20px 30px 40px 50px']);
		$expected = ['T' => '10px', 'R' => '20px', 'B' => '30px', 'L' => '40px'];
		$this->assertEquals($expected, $result);
	}
	...
	public function testSetCSSboxshadow_WithInset()
	{
		...
		$result = $this->cssManager->setCSSboxshadow('inset 5px 5px 10px #000');
		$this->assertCount(1, $result);
	...
	public function testInlinePropsToCSS_WithFontFamily()
	{
		...
		$this->invokeMethod($this->cssManager, 'convertInlinePropertiesToCss', [$bilp]);
		$p = $this->getCssProperties();
	...
	public function testInlinePropsToCSS_WithOTLtagsFFPlusNumeric()
	{
		...
		$this->assertArrayHasKey('FONT-FEATURE-SETTINGS', $p);
		$this->assertEquals("'salt' 2", $p['FONT-FEATURE-SETTINGS']);
	}
	...
	public function testInlinePropsToCSS_WithOTLtagsCombined()
	{
		...
		$this->assertArrayHasKey('FONT-FEATURE-SETTINGS', $p);
		$this->assertStringContainsString("'liga' 0", $p['FONT-FEATURE-SETTINGS']);
		$this->assertStringContainsString("'calt' 1", $p['FONT-FEATURE-SETTINGS']);
		$this->assertStringContainsString("'dlig' 0", $p['FONT-FEATURE-SETTINGS']);
		$this->assertStringContainsString("'salt' 3", $p['FONT-FEATURE-SETTINGS']);
	}
