...
class FpdiEncryptedSourceTest extends PdfUaTestCase
{
	...
	/**
	 * The kill-switch config flag $fpdiSkipEncryptedStringSanityCheck = true
	 * causes verifyAndPrepareMerge() to return true unconditionally even for
	 * a junk /Alt source, so callers hitting a false positive on legitimate
	 * non-Latin content can opt out without code changes.
	 */
	public function testSkipFlagBypassesSanityCheck()
	{
		$this->garbageAltPdf = $this->makeGarbageAltPdf();

		$mpdf = $this->makeMpdf([
			'PDFUAauto'                          => true,
			'fpdiSkipEncryptedStringSanityCheck' => true,
		]);
		$mpdf->setSourceFile($this->garbageAltPdf);
		$pageId = $mpdf->importPage(1);
		...
		$this->assertTrue(
			$merger->verifyAndPrepareMerge($pageId),
			'Sanity check must be bypassed when fpdiSkipEncryptedStringSanityCheck=true'
		);
		$this->assertFalse(
			$merger->wasSanityCheckFailed($pageId),
			'wasSanityCheckFailed() must remain false when the gauntlet is bypassed'
		);
	}
