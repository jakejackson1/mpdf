...
class HighBugRegressionsTest extends PdfUaTestCase
{
	...
	// =================================================================
	// HIGH-5 — PDFUA without useActiveForms produces untagged real content
	// =================================================================

	/**
	 * Strict mode (PDFUAauto=false) must throw at construction time when
	 * PDFUA is on but useActiveForms is off. Otherwise <input>/<select>/
	 * <textarea> render as drawn chrome that bypasses StructureTree, which
	 * fails veraPDF rule 7.1#3 (untagged real content) — the auto-mode
	 * audit found this as a HIGH-severity blocker.
	 */
	public function testStrictModeThrowsOnPdfuaWithoutActiveForms()
	{
		$this->expectException('\Mpdf\MpdfException');
		$this->expectExceptionMessageMatches('/useActiveForms/');
		// Bypass makeMpdf() — its defaults set useActiveForms=true.
		new \Mpdf\Mpdf([
			'PDFUA'          => true,
			'PDFUAauto'      => false,
	...
	/**
	 * PDFUAauto=true must auto-flip useActiveForms to true and record a
	 * warning, rather than throwing. This is the user-friendly fallback so
	 * sloppy callers still get a conformant document.
	 */
	public function testAutoModeFlipsUseActiveFormsAndWarns()
	{
		$mpdf = new \Mpdf\Mpdf([
			'PDFUA'          => true,
			'PDFUAauto'      => true,
			'title'          => 'Auto Active Forms',
			'mode'           => 'en-GB',
			'useActiveForms' => false,
		]);
		// useActiveForms must now report true.
		$this->assertTrue($mpdf->useActiveForms, 'PDFUAauto must flip useActiveForms to true');

		// A warning citing useActiveForms must have been recorded.
		$warnings = $mpdf->getPdfUaWarnings();
		$found = false;
		foreach ($warnings as $w) {
			if (stripos($w, 'useActiveForms') !== false) {
				$found = true;
				break;
			}
		}
		$this->assertTrue($found, 'PDFUAauto must record a useActiveForms warning');
	}

	/**
	 * When useActiveForms=true the constructor must succeed silently — no
	 * spurious warning, no throw. The guard is targeted at the configuration
	 * mistake, not at every PDFUA document.
	 */
	public function testCorrectConfigDoesNotEmitFormsWarning()
	{
		$mpdf = new \Mpdf\Mpdf([
			'PDFUA'          => true,
			'PDFUAauto'      => true,
		...
		$warnings = $mpdf->getPdfUaWarnings();
		foreach ($warnings as $w) {
			$this->assertStringNotContainsString('useActiveForms', $w);
		}
	}
