...
class ImageMapTest extends PdfUaTestCase
{
	...
	/**
	 * <map> / <area> populate $mpdf->pdfUaImageMaps even when no <img> uses them.
	 *
	...
	public function testMapAndAreaPopulateRegistry()
	{
		...
		$this->assertArrayHasKey('rooms', $mpdf->pdfUaImageMaps);
		$this->assertCount(2, $mpdf->pdfUaImageMaps['rooms']);
		$this->assertSame('rect', $mpdf->pdfUaImageMaps['rooms'][0]['shape']);
		$this->assertSame('Lobby', $mpdf->pdfUaImageMaps['rooms'][0]['alt']);
		$this->assertSame([10.0, 10.0, 100.0, 100.0], $mpdf->pdfUaImageMaps['rooms'][0]['coords']);
		$this->assertSame('circle', $mpdf->pdfUaImageMaps['rooms'][1]['shape']);
		$this->assertSame([200.0, 200.0, 40.0], $mpdf->pdfUaImageMaps['rooms'][1]['coords']);
		$this->assertNull($mpdf->pdfUaCurrentMapName);
	}
	...
	public function testAreaOutsideMapIsIgnored()
	{
		...
		$this->assertSame([], $mpdf->pdfUaImageMaps);
		$warnings = $mpdf->getPdfUaWarnings();
		$found = false;
		foreach ($warnings as $w) {
