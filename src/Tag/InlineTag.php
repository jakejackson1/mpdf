...
abstract class InlineTag extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		// The InlineUaStruct stack stores the *number* of struct elements that
		...
		if (!isset($this->mpdf->InlineUaStruct[$tag])) {
			$this->mpdf->InlineUaStruct[$tag] = [];
		}
		$this->mpdf->InlineUaStruct[$tag][] = $this->openInlineUaStruct($attr) ? 1 : 0;
	}
	...
	{
		$tag = $this->getTagName();
		if (!isset($this->mpdf->InlineUaStruct[$tag]) || empty($this->mpdf->InlineUaStruct[$tag])) {
			return;
		}
		$idx = count($this->mpdf->InlineUaStruct[$tag]) - 1;
		$this->mpdf->InlineUaStruct[$tag][$idx] += $count;
	}
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if (isset($this->mpdf->InlineUaStruct[$tag]) && count($this->mpdf->InlineUaStruct[$tag])) {
			$depth = (int) array_pop($this->mpdf->InlineUaStruct[$tag]);
			for ($i = 0; $i < $depth; $i++) {
