...
* PDF/UA-1: HTML ruby annotation tags (`<ruby>`, `<rb>`, `<rt>`, `<rp>`, `<rtc>`) are now parsed instead of stripped — added to `Mpdf::DisableTags()` `enabledtags` and given handler classes in `src/Tag/`. v1 (Span fallback): each ruby part maps to the `Span` standard struct type via `StructType::$tagMap`; `Tag/Ruby.php` and `Tag/Rt.php` push a `Span` struct element unconditionally so the annotation has its own tagged-tree handle, while `Tag/Rb.php`, `Tag/Rp.php`, and `Tag/Rtc.php` use the bare `InlineTag` machinery (lang/aria-label still produce a Span as elsewhere). Closes the audit-2026-05-01 L4 semantic-loss observation by guaranteeing every ruby part is tagged as real content (ISO 14289-1 §7.1) rather than dissolving into the surrounding block's struct element. **v2 deferred**: proper `Ruby`/`RB`/`RT`/`RP` standard struct types (ISO 32000-1 §14.8.5.6 Tables 339, 340) plus rt-above-rb visual stacking are gated on a separate layout-engine RFC (plan 2026-05-01 §4b); mPDF still flows the rt linearly inline today. Six new probes in `PoorHtmlAutoModeTest` (ruby with rp fallbacks, ruby inside `<h1>`/`<a href>`, bare-text base, nested ruby, `<rtc>`) plus a struct-element regression in `StructureElementsTest` and a tag-map unit test in `StructTypeTest` (@jakejackson1)
