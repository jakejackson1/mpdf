...
New features
------------
* PDF/UA-1: parsing/behaviour hygiene follow-ups from the P3 review (audit E-P3a). **Parsing change:** the default `enabledtags` whitelist in `Mpdf::DisableTags()` now permanently includes `<abbr>`, `<area>`, `<map>`, `<rb>`, `<rp>`, `<rt>`, `<rtc>` and `<ruby>` for *every* document (not just PDF/UA-1 ones) — HTML that previously had these tags stripped by `strip_tags()` now keeps them; the ruby/abbr/image-map handlers already no-op their PDF/UA-specific struct wiring when `PDFUA` is off, so non-UA rendering is otherwise unchanged. `Mpdf::AddCustomProperty()`'s empty-/over-127-byte key rejection is now scoped to `PDFUA` documents, restoring the prior "accept any key" behaviour for non-UA callers. `Tag\A::open()` regains the pre-UA fallback that applies anchor inline CSS for a non-hyperlink `<a href="">` when `PDFUA` is off (the `isHyperlink` gate only governs Link-annotation creation, never styling). `MetadataWriter` now tests the document title with `=== '' || === null` rather than `empty()`, so a literal title `"0"` is no longer treated as missing (previously dropped `dc:title`/`/Title` and could throw the strict-mode "requires a document title" error). `Tag\Area::open()` guards its `UaState` dereference to match the null-checks used further down. Defence-in-depth: `UaPolicy::normaliseHref()` folds case with a byte-safe `strtr()` A-Z→a-z table instead of the locale-dependent `strtolower()`, so the `javascript:`/`vbscript:` scheme deny-list cannot be dodged under an exotic locale on PHP < 8. (@jakejackson1)
* PDF/UA-1: performance and duplication hygiene from the P3 review (audit E-P3c) — behaviour-preserving, no output change. `StructureWriter::buildPageRefMap()` (a full `pageDim` scan) was rebuilt once per struct element during the tree walk; it is now built once per `writeStructTree()` and cached in `$pageRefMap`, making the walk O(elements + pages) instead of O(elements × pages). The copy-pasted `registerId()` + aria-`queue()` loop is folded into one `AriaIdResolver::queueAriaRefs($elem, $attr, $objattr)` helper shared by `Tag\A`, `Tag\InlineTag` and the Figure/barcode/text-circle image paths in `Mpdf` (the `$objattr` flag selects the `pdfua_`-prefixed key convention, since an image's `$objattr['ID']` is an unrelated Form-XObject number). The SVG `<title>`/`<desc>` → `/Alt` promotion (with its `''`/`null`/text triage) is shared between `printobjectbuffer()` and `Image()` via a new `Mpdf::svgAccessibleAlt()`. `Tag\BlockTag::close()` now calls the shared `Mpdf::restoreFlowingBlockPdfuaState()` (made public) instead of re-implementing it inline; `MetadataWriter` hoists the identical `'Internal link'` `/Contents` fallback out of its duplicated branches; and `ImageMapRegistry` takes the `UaState` facade via a plain `setUaState()` setter (matching `StructureTree`) rather than a lazy resolver closure. The one-`withArtifact(callable)` artifact-bracket consolidation was **skipped**: its call sites span three different emission mechanisms (`writer->write`, string `.=`, `headerbuffer .=`) with divergent BMC/BDC payloads and many early-return paths, so a single wrapper risked changing output. (@jakejackson1)
* PDF/UA-1: `/Layout` `/BBox` and numeric `/A` attribute values are now written locale-independently (audit E24). `StructureWriter::buildAttrObject()` serialised the `/BBox` array (via `implode(' ', $value)`) and every numeric attribute by raw string concatenation of PHP floats; because a bare float-to-string cast honours `LC_NUMERIC`, a document produced under a comma-decimal locale (e.g. `de_DE`, `nl_NL`) emitted `1,5` and corrupted the PDF number array — an invalid `/BBox` that breaks the `/Layout` bounding box (ISO 32000-1:2008 Table 344 / Matterhorn 13-008). Numerics now route through a `formatNumber()` helper that emits integers verbatim (always locale-safe) and floats via `sprintf('%.3F', …)` (uppercase `F` is locale-independent), trimming trailing zeros so integral coordinates stay compact — mirroring how the other writers format coordinates. New `testBBoxIsLocaleIndependent` in `tests/Mpdf/Ua/StructureWriterTest.php` sets a comma-decimal locale and asserts the `/BBox` array uses `.`; the veraPDF conformance gate still passes. (@jakejackson1)
* PDF/UA-1: the ParentTree NumTree now survives sparse `/StructParents` keys and sparse MCIDs (audit E23). `StructureWriter::writeParentTree()` built each page-level `/Nums` value array by appending one ref per entry as it iterated the (ksort-ed) MCID map, so the value array was correct only when the MCID keys were dense and 0-based — the array position is the MCID (ISO 32000-1:2008 §14.7.4.4). Imported MCRs (`StructureTree::registerImportedMcr()`, the Tier 2 FPDI merge) copy source-PDF MCIDs verbatim, so a key's map can be non-zero-based or contain a gap; a positional append then shifted every later ref by one slot, mis-mapping the surviving MCIDs to the wrong struct elements. The builder now walks `0..maxMcid` explicitly and places each ref at its own index, emitting `null` for any MCID with no owner so the remaining positions stay aligned. Dense 0-based maps (the common case) produce byte-identical output with no padding. The outer `/Nums` key sequence was already emitted as explicit `key value` pairs, so non-contiguous `/StructParents` keys were already preserved; a regression test now guards both. New `tests/Mpdf/Ua/StructureWriterTest.php` asserts the gap-preserving `null` padding, non-contiguous outer keys, and the unchanged dense-map bytes; a `testMultiKeyParentTreePassesUa1` case extends the veraPDF conformance gate. (@jakejackson1)
* PDF/UA-1: optional-content entry points now share one version/availability guard so PDF/A and PDF/X preserve their declared base version (audit E22). `Mpdf::SetVisibility()` only warned-and-returned for PDF/A / PDF/X when `$this->visibility` (the *current* state) was already non-visible, so the *first* `SetVisibility('printonly'|'screenonly'|'hidden')` call under PDF/A or PDF/X slipped past the guard, emitted an `/OC … BDC` marked-content group and set `hasOC` — an optional-content feature that PDF/A-1 and PDF/X forbid — even though `BeginLayer()`'s sibling guard blocked layers unconditionally. Both call sites now route through a private `allowOptionalContent($warning)` helper that records the `PDFAXwarnings` entry and tells the caller to skip the OCG under PDF/A / PDF/X, and otherwise bumps the base version to 1.5 for the OCG feature *except* under PDF/A, PDF/X or PDF/UA whose declared base versions must be preserved (PDF/UA-1 is PDF 1.7, ISO 14289-1:2014 §6). `SetVisibility()` now guards on the requested `$v` rather than the stale current state, so any non-visible visibility is blocked from the first call and `SetVisibility('visible')` stays permitted. New `tests/Mpdf/Ua/OptionalContentVersionTest.php` covers PDF/A, PDF/X, PDF/UA (1.7 preserved), plain-PDF (1.5 advertised) and the shared `BeginLayer()` path. (@jakejackson1)
* PDF/UA-1: `OverWrite()` now honours the strict/auto two-path contract instead of throwing in both modes (audit E21). The guard tested only `$this->PDFUA`, so `['PDFUA'=>true,'PDFUAauto'=>true]` threw an `\Mpdf\MpdfException` even though `PDFUAauto` promises to warn-and-proceed — mirroring the `SetProtection()` divergence. Because binary string replacement cannot maintain the logical structure tree required for accessibility, strict mode (`PDFUAauto=false`) still throws so the caller knows the guarantee is lost, but auto mode now records a warning via `getPdfUaWarnings()` (the accessibility guarantee is a no-op for that call) and performs the overwrite rather than aborting. New `tests/Mpdf/Ua/PdfUaModeTest.php` covering both paths. (@jakejackson1)
* PDF/UA-1: polygon image-map hotspots now tile their interior with `/QuadPoints` instead of collapsing to the bounding box (audit E20). `ImageMapRegistry::shapeToRect()` reduced every `poly`/`polygon` `<area>` to its bounding rectangle, so the emitted Link annotation activated a large non-hotspot region (e.g. a triangular room selecting the whole square around it). The registry now ear-clips the polygon into triangles (`triangulatePolygon()`, John W. Ratcliff's algorithm — convex and concave simple polygons) and maps each triangle through the same pixel→device affine the rotated maps already use (the C2 `/QuadPoints` machinery: `buildHotspotMatrix()`, the shared `emitQuads()`, and the `Mpdf::Link()` quad-points slot), emitting one degenerate quad per triangle so the union of quads approximates the polygon; `/Rect` stays the bounding box. A synthesised axis-aligned placement matrix (`buildAxisAlignedMatrix()`) extends the tiling to un-rotated hosts too. A non-simple/degenerate polygon that cannot be triangulated falls back to the bounding-box path so the link still emits. New `testTriangularPolyEmitsQuadPointsCoveringTriangle` in `tests/Mpdf/Ua/ImageMapTest.php` and `testPolygonImageMapPassesUa1` on the veraPDF conformance gate. (@jakejackson1)
* PDF/UA-1: image-map violations now honour the branch's strict/auto contract instead of warning unconditionally (audit E19). `ImageMapRegistry::drain()`/`emitForImage()` and the `>1024`-iteration internal-link disambiguation guard all called `warn()` (→ `addWarning()`) regardless of mode, and the registry never read `PDFUAauto` — so an `<img usemap="#missing">` pointing at an unknown `<map>`, a malformed `<area coords>`, and an anchor-collision blow-out only ever warned, even in strict mode, silently emitting partial or mis-placed link output. Those three call sites now route through a new `enforce()` helper that mirrors the tag handlers (`Tag\Area::open`, `Tag\A::open`): in strict mode (`PDFUAauto=false`) it throws `\Mpdf\MpdfException` naming the offending map/area, and in `PDFUAauto` mode it records the diagnostic via the lazy `UaState` resolver and skips the offending item — so strict fails loudly while auto stays conformant. New `testStrictModeImgUsemapUnknownMapThrows` in `tests/Mpdf/Ua/ImageMapTest.php`; the existing unknown-map warn test now runs under explicit `PDFUAauto=true`. (@jakejackson1)
* PDF/UA-1: resolved ARIA relationship attributes are now emitted or visibly warned instead of stored-and-dropped (audit E18). `AriaIdResolver::resolveAll()` resolved `aria-controls` / `aria-owns` / `aria-flowto` / `aria-activedescendant` to their target struct element and stashed them under `attributes['_aria_relationships']`, but no `StructureWriter` path ever read that key — so every relationship was silently dropped and the internal marker key sat unused in the attribute map. The four are now split by whether PDF can express them: `aria-owns` / `aria-controls` map to a PDF `/Ref` entry (ISO 32000-2 §14.7) — an array of indirect references to the struct elements the referring element refers to — which `StructureWriter::writeElement()` emits as `/Ref [N 0 R …]` (targets' object numbers are pre-allocated by `reserveObjectNumbers()`, so forward and cross-tree references resolve, and duplicate targets collapse to one entry); `aria-flowto` (a reading-order override, which a tagged PDF derives from structure-tree order) and `aria-activedescendant` (a transient interactive-focus relationship) have no static PDF/UA-1 representation and are surfaced through `getPdfUaWarnings()` in both strict and `PDFUAauto` mode rather than store-and-dropped (they are not conformance violations, so strict does not throw). The relationships are held in a dedicated `StructureElement::$relationships` list off the `$attributes` map, so no `_aria_*` marker key can leak into an emitted `StructElem` dict. New `tests/Mpdf/Ua/AriaRelTest.php`; a document carrying `aria-controls` and its `/Ref` was verified PDF/UA-1 compliant with veraPDF. (@jakejackson1)
* PDF/UA-1: tables now emit their `THead`/`TBody`/`TFoot` row-grouping elements instead of hanging `TR` straight off `Table` (audit E16). `Tr::open()` pushed every `TR` directly beneath the enclosing `Table` and the `Thead`/`Tbody`/`Tfoot` tag handlers created no struct element at all — parked "when those are supported" — so the row-grouping (and `THead` repetition) semantics of ISO 32000-1:2008 §14.8 Table 333 were lost for every tagged table. The section handlers now open a `THead`/`TBody`/`TFoot` struct element and nest their `TR` rows beneath it; when the HTML writes rows straight under `<table>`, `Tr::open()` synthesises a single `TBody` to hold the group-less rows (collapsed at the next explicit row group), and `StructureTree::closeRowGroup()` backstops `Table::close()` so a group left open by an omitted optional end tag — or the synthetic `TBody` — is popped before the `Table` itself. So `<table><thead><tr>…</tr></thead><tbody><tr>…</tr></tbody><tfoot><tr>…</tr></tfoot></table>` now produces `Table ▸ THead ▸ TR`, `Table ▸ TBody ▸ TR`, `Table ▸ TFoot ▸ TR`, and a nested table keeps its own row group. New `tests/Mpdf/Ua/TableStructureTest.php` plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: an imported **untagged** PDF page is now signalled and can be named instead of vanishing into an anonymous artifact (audit E15). `FpdiTrait::useImportedPage()`'s Tier 1 path wrapped the whole imported page in `/Artifact <</Type /Layout>> BDC … EMC` — conformant (Matterhorn 01-007: real content marked as an artifact) but wholly inaccessible, and it did so silently in **both** modes, which the no-deferrals rule treats as a gap. The path is now a supported, signalled behaviour: in `PDFUAauto` mode the page is still wrapped as an artifact **and** `getPdfUaWarnings()` records an entry naming the (redacted) source file and page number, so the content loss is no longer silent; in strict mode `useImportedPage()` throws `\Mpdf\MpdfException` naming the same source page and citing Matterhorn 01-007, so the producer must supply a tagged source or an explicit accessible name. New: passing `useImportedPage($id, ['alt' => 'description'])` tags the entire imported page as a captioned `Figure` struct element carrying that `/Alt` (ISO 14289-1:2014 §7.3) — a named, accessible artifact — which works in strict mode without a throw (a blank/whitespace `alt` is treated as no name). `importPage()` records the redacted source key + page number per imported `pageId` (never a full filesystem path — audit L-5/I-6) so the warning and throw can name the page. New `tests/Mpdf/Ua/FpdiImportTest.php` plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: header-cell (`<th>`) associations are now built directly on the `TH` element (audit E12). `Th::open()` replayed `Td::open()`, which registered the HTML `id` against a *temporary* `TD` struct element and parsed `headers="…"`, then discarded that `TD` (`discardTop()`) and rebuilt a `TH` keeping only `Scope`/`ColSpan`/`RowSpan`. Because `registerId()` is first-wins, the `id` bound to the orphaned `TD` (whose `objNum` stayed `0`) and the real `TH` never got it; the `headers="…"` associations were dropped on the rebuilt `TH`; and `Th::open()`'s PDFUA block had no artifact-scope guard, so a `<th>` in a running header/footer attached its `id` to the `Document` root. `Td::open()`'s struct wiring is now factored into an overridable `pdfuaOpenCellStruct()` with three hooks (`pdfuaCellStructType()`, `pdfuaCellStructAttrs()`, `pdfuaRegisterCellId()`); `Th` overrides them to build the `TH` **once** and register its `id`/`Headers`/`Scope` against it, and the shared method now applies the `isInArtifact()` guard the other cell tags use — so a `<th id scope>` carries `/A <</O /Table /Scope …>>` and its `/ID`, a `<td headers>` (or a `<th headers>`) carries the matching `/Headers` reference, the `id` resolves to the `TH` with a non-zero object number, and a `<th>` in a running header renders as pagination artifact. New `tests/Mpdf/Ua/TableHeadersTest.php` plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: an `<img>` with no `alt` but an ARIA/`title` accessible name is now named instead of aborting or being hidden (audit E11). The `alt`-absent branch of `Mpdf::printobjectbuffer()` threw (strict) or demoted the image to a decorative `/Artifact` (`PDFUAauto`) *without* consulting the ARIA naming attributes — the `aria-labelledby`/`aria-describedby` queue lived only in the non-empty-`alt` branch — so `<img src="chart.png" aria-labelledby="figcap">` (no `alt`) lost its accessible name even though one was available. The alt-absent branch now checks the accessible-name sources first, in WAI-ARIA name-computation precedence (`aria-labelledby` > `aria-label` > `title`): a direct-string `aria-label`/`title` is promoted straight into the Figure's `/Alt`, and an `aria-labelledby` reference opens the `Figure` with only `/BBox` and lets the existing `AriaIdResolver` second pass fill `/Alt` from the target's real text at `_enddoc()` (reusing the audit-E8 resolution path — no copy-paste). `Img::open()` now carries `aria-label`/`title` through `$objattr` alongside the ID-referencing ARIA attributes. Only when *no* name source exists does strict mode still throw / `PDFUAauto` still demote to `/Artifact` (`aria-describedby`/`-details` are descriptions → `/E`, not a name, so they alone do not rescue the image). New `DirectPhpAndAriaTest` cases plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: block-level tags inside a table cell now open their real struct element (audit E9). `BlockTag::open()`'s struct-element creation was gated on `!$this->mpdf->tableLevel`, so `<td><h2>…</h2><ul><li>…</li></ul></td>` produced only `Table▸TR▸TD` with the cell's whole content collapsed under a single `TD` MCID — no `H2`, no `L`/`LI` — and the §7.4.2 heading-sequence tracker never saw the cell's headings (common in invoices/reports; silent in both modes). Block tags in a cell now open their normal struct element beneath the current `TD`/`TH` and register their `id`/ARIA relations, and the deferred cell render attributes each chunk's MCID to its true owner: `_saveCellTextBuffer()` captures the innermost open struct element per chunk (the `TD`/`TH`, or a nested `H2`/`L`/`LI`/`Link`/lang-`Span`), and the `printbuffer()` emit loop brackets each chunk in that element's own BDC (reusing the audit-E6 `ensureInlineBdcOpen()` path) rather than one blanket `TD` MCID — so `TD` owns no direct content when all of it lives in children, a rotated cell keeps its single flattened `TD` MCID, and cell headings take part in the one document-wide heading sequence (strict mode throws / `PDFUAauto` clamps on a bad sequence). New `tests/Mpdf/Ua/TableCellStructureTest.php` plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: inline struct elements now own their own content instead of an empty `/K` (audit E6). Every flowing-text MCID was attributed to the enclosing **block** element (a single per-line `/P` BDC), so an inline `Link` / lang-`Span` / `Abbr` `/E`-Span / Ruby `RB`·`RT` owned no content item: the `Link` carried only its annotation OBJR, and the inline element's `/Lang`, `/Alt` or `/E` applied to nothing — which veraPDF flags as an empty `Link`/`Span` (ISO 14289-1 §7.18.5 / §7.2, Matterhorn 02-003 / 11-001) and drops the accessible metadata. The layout engine now tracks the innermost inline struct element per text run (`StructureTree::getCurrentInline()`, carried through the textbuffer and `saveFont()`/`restoreFont()` cycle on `AnchorState`) and brackets that run's marked content in the inline element's **own** BDC via `Mpdf::ensureInlineBdcOpen()`, attributing the MCID to that element and resuming the block's BDC once the inline run ends. So `<p>see <a href="…">the report <span lang="fr">rapport</span></a> today</p>` now emits a `/Link` element whose `/K` holds both the link-text MCID and the OBJR, a lang-`Span` that owns the "rapport" MCID, and `/P` MCIDs for the surrounding "see"/"today"; `<abbr>`'s `/E`-Span and Ruby `RB`/`RT` likewise own their text. New `tests/Mpdf/Ua/InlineStructContentTest.php` plus a veraPDF conformance case. (@jakejackson1)
* PDF/UA-1: `aria-labelledby` / `aria-describedby` / `aria-details` now resolve to the target's real text instead of a content-hiding empty `/Alt` (audit E8). `AriaIdResolver::collectText()` was a stub that returned only the target element's own `/ActualText` or `/Alt`; because descendant text is stored as MCIDs rather than text-node children, an ordinary text target returned `""` and `resolveAll()` wrote that empty string as `/Alt (\376\377)` — a BOM-only value that, per ISO 32000-1 Table 322, *replaces* the referring element's content for assistive technology and silently hides it. The structure tree now retains each block's text as it is laid out (`StructureElement::appendText()`, populated from the layout engine's per-line content and read back via `getOwnText()`), and `collectText()` concatenates the target's own text with its descendants' to build the accessible name (e.g. `<p id="cap">The caption</p><div aria-labelledby="cap">` now emits `/Alt` = "The caption"). A naming reference whose target is missing or carries no text never emits an empty `/Alt`/`/E`: strict PDF/UA mode throws and `PDFUAauto` mode warns (Matterhorn 13-004 / 28-002). New `tests/Mpdf/Ua/AriaNameResolutionTest.php` plus a veraPDF conformance case; two mode-independent `AriaDosTest` cases were moved to `PDFUAauto` so their dangling references exercise the DoS caps without the new strict throw. (@jakejackson1)
